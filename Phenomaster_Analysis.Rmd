...
TSE.data.clean <- read_delim("C:/Users/jens_/OneDrive/Dokumente/Master/Master Thesis/Experiments/JS22 001/Data/clean_data_JS22_001.csv", 
                      delim = ",",
                      col_names = TRUE, 
                      col_types = cols(
...
metadata <- read_delim("C:/Users/jens_/OneDrive/Dokumente/Master/Master Thesis/Experiments/JS22 001/Data/JS22_001_metadata.csv", 
                      delim = ";",
                      col_names = TRUE, 
                      col_types = cols(
                        Box = col_factor(levels=NULL, ordered = FALSE)),
...
ggplot(data=circadian.energy.d, aes(x = day.num, y = circadian.Drink.d, col = mouse_ID), color=genotype) +
  geom_point(size=2) + geom_line() + facet_wrap(~genotype) +
  theme_light() +
  ylab("Daily Drink Consumption") + xlab("Day") 
...
prawEE<-ggplot(data = avg.rawEE.pm)+
  coord_cartesian(ylim = c(0.29,0.6), xlim = c(0,24))+ #scale_y_limit drop datapoints, coord_cartesian doesnt
...
pdf("RawEE.pdf",width = 5,height = 4)
prawEE
dev.off()
...
prawEE_t <- ggplot(avg.rawEE.pm.t)+
  ...
  scale_y_continuous(limit=c(0.15,0.65))+
...
pdf("RawEE_t.pdf",width = 8,height = 4)
prawEE_t
dev.off()
...
#RER during the circadian cycle
  pRER<-ggplot(data = avg.RER.pm)+
   coord_cartesian(ylim = c(0.7,1), xlim = c(0,24))+ 
    scale_x_continuous(breaks = c(0,6,12,18,24),expand = c(0,0))+
    annotate("rect", ymin = -Inf, ymax = Inf, xmin=12, xmax=24, alpha = 0.2,fill="grey")+
    annotate("rect", ymin = -Inf, ymax = 0.68, xmin=0, xmax=12,fill="white",col="black")+
    annotate("rect", ymin = -Inf, ymax = 0.68, xmin=12, xmax=24,fill="black",col="black")+
    theme_bw() +
    theme(panel.grid.major.x=element_blank())+
  ...
  pdf("RER_all.pdf",width = 5,height = 4)
  pRER
  dev.off()
  
  pdf("RER_dayvsnight.pdf",width = 4,height = 4)
  pRERd+pRERn
  dev.off()
